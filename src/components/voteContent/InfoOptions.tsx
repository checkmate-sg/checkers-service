        ...
        >
          {numberList.map(num => (
            <div key={num} className="flex items-center justify-center gap-2">
              <RadioGroupItem value={String(num)} id={`truth-score-${num}`} className="h-5 w-5" />

              <Label htmlFor={`truth-score-${num}`}>{num}</Label>
            </div>
          ))}
        </RadioGroup>
      </div>
  ...
  return (
    <div>
      <div className="text-primary text-justify my-3">
        Please assess the veracity of the claim(s) in the message on a scale from 0 (entirely false)
        to 5 (entirely true).
      </div>
      <TruthScoreOptions
