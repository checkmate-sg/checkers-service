...
export const ALL_RESPONSE_CATEGORIES: string[] = ["great", "acceptable", "unacceptable", null];
...
export async function getCategoryCountsByPollId(pollId: string) {
  ...
  try {
    ...
    const rawCategoryCount = await env.CHECKERS_DB_SERVICE.getVotesDetails(
      pollId,
      categoryPipeline
    );
...
export async function getTotalVoteRequestsCount(pollId: string) {
  ...
  try {
    ...
    const totalVotePipeline: any[] = [{ $count: "totalVoteRequestCount" }];

    const rawTotalVoteRequestsCount = await env.CHECKERS_DB_SERVICE.getVotesDetails(
      pollId,
      totalVotePipeline
    );
...
export async function getTotalTruthScoreValue(pollId: string) {
  ...
  try {
    ...
    const rawTotalTruthScoreValue = await env.CHECKERS_DB_SERVICE.getVotesDetails(
      pollId,
...
export async function getResponseCategoryCountsByPollId(pollId: string) {
  ...
  try {
    ...
    const rawResponseCategoryCount = await env.CHECKERS_DB_SERVICE.getVotesDetails(
      pollId,
      responseCategoryPipeline
    );
